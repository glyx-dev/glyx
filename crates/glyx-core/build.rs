...
fn main() {
    println!("cargo:rerun-if-env-changed=GLYX_APP_SNAPSHOT");
    println!("cargo:rerun-if-env-changed=GLYX_APP_JS");
    println!("cargo:rerun-if-env-changed=GLYX_APP_CONFIG");

    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR not set"));
    let generated = out_dir.join("embedded_snapshot.rs");

    let snapshot_const = embed_bytes("GLYX_APP_SNAPSHOT", "EMBEDDED_SNAPSHOT");
    let app_js_const   = embed_str("GLYX_APP_JS", "EMBEDDED_APP_JS");
    let config_const   = embed_str("GLYX_APP_CONFIG", "EMBEDDED_CONFIG");
