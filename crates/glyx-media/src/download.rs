//! Download and cache the glyx-media DLL from the CDN.
//!
//! CDN layout:
//!   https://cdn.glyx.dev/media/{version}/glyx-media-{platform}-{arch}.{ext}
//!   https://cdn.glyx.dev/media/{version}/glyx-media-{platform}-{arch}.manifest.json
//!   https://cdn.glyx.dev/media/{version}/glyx-media-{platform}-{arch}.manifest.sig
//!
//! Local cache:
//!   ~/.glyx/cache/media/glyx-media-{version}-{platform}-{arch}.{ext}
//!   ~/.glyx/cache/media/glyx-media-{version}-{platform}-{arch}.manifest.json
//!   ~/.glyx/cache/media/glyx-media-{version}-{platform}-{arch}.manifest.sig

use std::path::PathBuf;
use crate::verify::{sha256_hex, verify_manifest};

/// Minimum glyx-media version this runner accepts.
/// Bump when the C ABI changes (major version) or new features are required.
pub const GLYX_MEDIA_VERSION: &str = "1.0.0";

const CDN_BASE: &str = "https://cdn.glyx.dev/media";
...
/// Download the glyx-media DLL for this platform and cache it.
...
pub async fn download_and_cache_media() -> Result<PathBuf, String> {
    let stem    = dll_stem();
    let ext     = dll_ext();
    let version = GLYX_MEDIA_VERSION;
    ...
    let manifest_bytes = client.get(&manifest_url).send().await
        ...
        .bytes().await
        .map_err(|e| format!("glyx-media: manifest read failed: {e}"))?
        .to_vec();

    let sig_bytes = client.get(&sig_url).send().await
        .map_err(|e| format!("glyx-media: sig fetch failed: {e}"))?
        .bytes().await
        .map_err(|e| format!("glyx-media: sig read failed: {e}"))?
        .to_vec();

    // 1. Verify signature on manifest before downloading the DLL.
    let manifest = verify_manifest(&manifest_bytes, &sig_bytes)?;

    log::info!("[glyx-media] manifest verified, downloading DLL ({dll_url})");

    let dll_bytes = client.get(&dll_url).send().await
        .map_err(|e| format!("glyx-media: DLL fetch failed: {e}"))?
        .bytes().await
        .map_err(|e| format!("glyx-media: DLL read failed: {e}"))?
        .to_vec();
    ...
    if actual_hash != manifest.sha256 {
        return Err(format!(
            "glyx-media: SHA-256 mismatch (expected {}, got {}) — download may be corrupted",
    ...
    std::fs::write(&dll_path, &dll_bytes)
        .map_err(|e| format!("glyx-media: cannot write DLL to cache: {e}"))?;
    std::fs::write(dll_path.with_extension("manifest.json"), &manifest_bytes)
        .map_err(|e| format!("glyx-media: cannot write manifest: {e}"))?;
    std::fs::write(dll_path.with_extension("manifest.sig"), &sig_bytes)
        .map_err(|e| format!("glyx-media: cannot write sig: {e}"))?;

    log::info!("[glyx-media] DLL cached at {}", dll_path.display());
    Ok(dll_path)
}
