//! glyx-renderer — Vello-based 2D rendering.
//!
//! ## Why FrameBuilder owns Scene (not borrows it)
//!
//! If FrameBuilder held &mut Scene from GlyxRenderer, that borrow would
//! keep &mut GlyxRenderer alive across the whole frame-build span.
...
pub struct GlyxRenderer {
    ...
    /// Set from `window.background` in glyx.config.json; defaults to
    /// the Glyx dark background so the window is never a blank white flash.
...
impl GlyxRenderer {
    pub fn new(gpu: &GpuContext, use_cpu: bool) -> Result<Self, RendererError> {
        if use_cpu {
            log::warn!("glyx-renderer: CPU rendering active.");
        }
        ...
        // glyx-gpu enables it only when the adapter advertises support.
        //
                ...
                Self::pipeline_cache_path().map(|path| {
                    ...
                    let cache = unsafe {
                        gpu.device.create_pipeline_cache(&wgpu::PipelineCacheDescriptor {
                            label:    Some("glyx-pipeline-cache"),
                            data:     seed.as_deref(),
                            fallback: true,
                        })
                    };
                    if seed.is_some() {
                        log::debug!("glyx-renderer: pipeline cache loaded from {:?}", path);
                    }
                    cache
                })
            } else {
    ...
    pub fn try_save_pipeline_cache(&self) {
        ...
        match std::fs::write(&path, &data) {
            Ok(_)  => log::debug!("glyx-renderer: pipeline cache saved ({} bytes)", data.len()),
            Err(e) => log::warn!("glyx-renderer: failed to save pipeline cache: {e}"),
        }
    }
...
/// glyx-core hold the builder and call renderer.render_frame() on the same
...
impl FrameBuilder {
    ...
    pub fn draw_text(&mut self, layout: &glyx_text::TextLayout,
...
impl AnyFrame {
    ...
    pub fn draw_text(&mut self, layout: &glyx_text::TextLayout, x: f64, y: f64, color: Color) {
        match self {
