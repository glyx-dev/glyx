...
//!
//! When `glyx dev --inspect` is set (via `GLYX_INSPECT_PORT` env var), this
...
//!
//! • Each frame, `GlyxInspector::pump_messages()` drains the inbox and
//!   dispatches messages to the V8 inspector session.  Responses flow back
//!   through the `GlyxChannel` → outbox → WebSocket.
//!
...
// SAFETY: GlyxInspector is only ever used on the V8 thread.
unsafe impl Send for GlyxInspector {}
...
impl GlyxInspector {
    ...
    /// `cdp_log_tx` is set so that `__glyx_log` can forward console messages
...
async fn run_ws_server(
    ...
    loop {
        ...
        if peek_str.contains("GET /json") {
            ...
            let body = format!(
                r#"[{{"type":"node","id":"glyx-1","title":"Glyx Dev","url":"file://glyx","webSocketDebuggerUrl":"ws://127.0.0.1:{port}/"}}]"#
            );
            let response = format!(
