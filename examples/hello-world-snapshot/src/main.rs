...
//!
//!   GLYX_SNAPSHOT_PATH=/tmp/hello-world.snapshot cargo run -p hello-world-snapshot
//!

fn main() {
    // Load snapshot blob from environment variable or default path
    let snapshot_path = std::env::var("GLYX_SNAPSHOT_PATH")
    ...
    glyx_core::run(glyx_core::AppConfig {
        window: glyx_core::WindowConfig {
            title:  "Glyx — Hello World (Snapshot)".into(),
            width:  1280,
            height: 800,
            ..Default::default()
        },
        ...
        extensions: vec![],
        js_plugins: vec![],
        dev_mode: Some(glyx_core::DevModeConfig::new(
            ...
            vec![
                ...
                std::path::PathBuf::from("js/packages/@glyx/react/src"),
            ],
        )),
    });
}
