...
No matching component was found for:
  ...
  function _glyxRequestFrame(ms) {
    ...
    if (globalThis._glyxFramePending && deadline >= globalThis._glyxFramePendingDeadline)
      return;
  ...
  if (typeof TextEncoder === "undefined") {
    let _glyxUtf8Bytes2 = function(codePoint) {
      if (codePoint < 128)
        return [codePoint];
      if (codePoint < 2048) {
        return [192 | codePoint >> 6, 128 | codePoint & 63];
      }
      if (codePoint < 65536) {
        return [
          224 | codePoint >> 12,
    ...
    _glyxUtf8Bytes = _glyxUtf8Bytes2;
    globalThis.TextEncoder = class TextEncoder2 {
      get encoding() {
        return "utf-8";
      }
      encode(input) {
      ...
      encodeInto(input, dest) {
        ...
        for (let i = 0;i < str.length; i++) {
          const code = str.codePointAt(i);
          const isPair = code > 65535;
          const bytes = _glyxUtf8Bytes2(code);
          if (written + bytes.length > dest.length)
            break;
          for (let k = 0;k < bytes.length; k++)
            dest[written + k] = bytes[k];
  ...
  var imageErrorRegistry = new Map;
  var systemWatchRegistry = new Map;
      ...
      switch (ev.type) {
            ...
            if (isPageKey || isJumpKey || isArrowKey) {
              ...
              if (target !== null) {
                ...
                if (k === "ArrowUp")
                  ...
                  sh.onScroll?.(-(viewH - LINE));
                else if (k === "PageDown")
                  sh.onScroll?.(viewH - LINE);
                else if (k === "Home")
                  sh.onAbsoluteScroll?.(0);
                else if (k === "End")
                  sh.onAbsoluteScroll?.(999999);
              }
              break;
            }
          }
        ...
        case "accessibilityValueChange": {
          ...
          if (ev.action === "increment")
            h.onIncrement?.();
          else if (ev.action === "decrement")
            h.onDecrement?.();
          else if (ev.action === "setValue" && ev.numericValue !== undefined)
            h.onSetValue?.(ev.numericValue);
          break;
        }
        case "ime": {
          if (focusedNodeId === null)
        ...
        case "cursorMoved": {
          ...
          if (inputDragNodeId !== null) {
            ...
            if (ih && ih.onDragAt) {
              ...
              if (layout)
                ih.onDragAt(ev.x - layout.x, ev.y - layout.y);
            }
          }
          break;
        }
        case "scroll": {
          ...
          for (const [nodeId, handlers] of scrollRegistry) {
            if (!hitTest(nodeId, cursorX, cursorY) || isDisabled(nodeId))
              continue;
          ...
          if (cb) {
            ...
            try {
              ...
              if (typeof __glyx_log !== "undefined")
                __glyx_log("[system.watch] callback error: " + e);
            }
          }
          break;
        }
  ...
  var _fsWatchCallbacks = new Map;
  function _pollFsWatch() {
    if (typeof __glyx_fs_watch_poll === "undefined")
      return;
    ...
    for (const ev of events) {
      ...
      if (cb)
        cb({ path: ev.path, type: ev.type });
    }
  }
  var _dbBackupTimers = new Map;
  class GlyxHeaders {
    constructor(init) {
      this._m = new Map;
      if (!init)
  ...
  function _pollVideo() {
    if (typeof __glyx_video_poll === "undefined")
      return;
  ...
  var _webviewCallbacks = new Map;
  function _pollWebview() {
    if (typeof __glyx_webview_poll === "undefined")
      return;
    const events = JSON.parse(__glyx_webview_poll());
    for (const ev of events) {
      const cbs = _webviewCallbacks.get(ev.id);
      if (cbs && cbs.onMessage)
        cbs.onMessage(ev.message);
    }
  }
  var webview = {
    postMessage(nodeId, message) {
      if (typeof __glyx_webview_post_message === "undefined")
        return;
  ...
  var _pendingRaycasts = new Map;
  function _pollCanvas3DRaycasts() {
    if (typeof __glyx_canvas3d_raycast_poll === "undefined")
      return;
    let raw;
    try {
      raw = __glyx_canvas3d_raycast_poll();
    } catch {
      return;
    }
    if (!raw || raw === "[]")
  ...
  class GlyxCanvas3DContext {
    ...
    unloadGltf(path) {
      if (typeof __glyx_canvas3d_unload_gltf === "undefined")
        return;
      __glyx_canvas3d_unload_gltf(path);
    }
    raycast(x, y, width, height) {
      if (typeof __glyx_canvas3d_raycast === "undefined" || !width || !height)
      ...
      try {
        ...
        return new Promise((resolve) => {
          _pendingRaycasts.set(reqId, resolve);
        });
      } catch (e) {
  ...
  var View = ({ children, style, ...props }) => import_react2.default.createElement("view", { style, ...props }, children);
  function Text({ children, style, showCursor, ...props }) {
    ...
    return import_react2.default.createElement("text", { text, style, showCursor, ...props });
  }
  function Pressable({ children, onPress, onRightPress, onPressIn, onPressOut, onHoverIn, onHoverOut, disabled, feedback = true, style, _glyxOnMount: externalOnMount, ...props }) {
    const nodeIdRef = import_react2.useRef(null);
    const handlersRef = import_react2.useRef(null);
    const [pressed, setPressed] = import_react2.useState(false);
    const [hovered, setHovered] = import_react2.useState(false);
    handlersRef.current = {
      onPress: (e) => {
    ...
    const onMount = import_react2.useCallback((id) => {
    ...
    return import_react2.default.createElement("view", { _glyxOnMount: onMount, style: mergedStyle, pressable: true, ...props }, children);
  }
  function useWindowSize() {
    const [size, setSize] = import_react2.useState(() => {
  ...
  var _SelectionCtx = import_react2.createContext(true);
  ...
  function PopoverHost() {
    const [, force] = import_react3.useState(0);
  ...
  var SelectColorsContext = import_react4.default.createContext(SELECT_COLORS_DARK);
  var _RadioCtx = import_react4.default.createContext(null);
  ...
  // ../../js/packages/@glyx/react/src/webview.js
  var import_react6 = __toESM(require_react(), 1);
  var WebView = import_react6.default.forwardRef(function WebView2({ src, html, sandbox = true, allowedOrigins, assetsRoot, onMessage, style, ...rest }, ref) {
    const nodeIdRef = import_react6.default.useRef(null);
    import_react6.default.useEffect(() => {
      ...
      return () => {
        _webviewCallbacks.delete(id);
      };
    }, [onMessage]);
    import_react6.default.useImperativeHandle(ref, () => ({
      get nodeId() {
        return nodeIdRef.current;
      },
      postMessage(message) {
        if (nodeIdRef.current != null)
          webview.postMessage(nodeIdRef.current, message);
      }
    }), []);
    const opts = {};
    if (sandbox === false)
    ...
    if (assetsRoot)
      opts.assetsRoot = assetsRoot;
    return import_react6.default.createElement("webview", {
  ...
  function render(element) {
    GlyxReconciler.updateContainer(import_react7.default.createElement(View, {
      style: { position: "relative", flexDirection: "column", flexGrow: 1, alignSelf: "stretch" }
    }, element, import_react7.default.createElement(PopoverHost)), rootContainer, null, null);
  }
