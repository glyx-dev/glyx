// @velox/auth — OAuth flows via system browser + deep-link callback, with
// tokens stored in the OS keychain.
//
// The app must be registered as the handler for its deep-link scheme (see
// `deeplink` in velox.config.json) and provide a token-exchange endpoint.
//
...
import { veloxWindow, deeplink } from '@velox/react';
...
export function createAuth({ redirect, providers = {}, storage } = {}) {
  ...
  async function signIn(name) {
    ...
    veloxWindow.openExternal(`${p.authUrl}?${params.toString()}`);
