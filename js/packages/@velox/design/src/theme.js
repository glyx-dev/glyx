// @velox/design — ThemeProvider + useTheme hook
//
// Usage:
//   import { ThemeProvider, useTheme } from '@velox/design';
//
//   // Wrap your app root:
//   render(
//     <ThemeProvider>
...
export function ThemeProvider({ colorScheme = 'system', overrides, children }) {
  const [isDark, setIsDark] = useState(() => {
    ...
    try {
      return typeof __velox_system_getDarkMode !== 'undefined'
        ? __velox_system_getDarkMode() === 'dark'
        : false;
    } catch { return false; }
  });
    ...
    const id = setInterval(() => {
      try {
        if (typeof __velox_system_getDarkMode !== 'undefined') {
          setIsDark(__velox_system_getDarkMode() === 'dark');
        }
      } catch {}
    }, 2000);
