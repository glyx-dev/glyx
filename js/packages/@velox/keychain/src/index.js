// @velox/keychain — Typed namespace-scoped OS keychain for Velox apps.
//
// Wraps @velox/react's `credentials` API with:
...
//
// Requires `credentials: true` in velox.config.json.
//
// Usage (simple):
...
import { credentials } from '@velox/react';
 ...
 * @param {{ service?: string }} opts  OS keychain service name (default 'velox')
 * @returns {{ set, get, delete, clear }}
 */
export function createKeychain(namespace, { service = 'velox' } = {}) {
  ...
  return {
    ...
    async set(key, value) {
      if (!(key in schema)) throw new Error(
        `@velox/keychain: unknown key "${key}" in namespace "${namespace}"`
      );
