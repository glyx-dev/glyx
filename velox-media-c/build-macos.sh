...
# build-macos.sh
#
# Builds velox-media.dylib for macOS (x64 or arm64, detected automatically).
#
# Prerequisites:
...
OUTPUT_NAME="velox-media-$VERSION-macos-$TARGET_ARCH.dylib"
...
clang -arch "$CLANG_ARCH" \
    -shared -fPIC -O2 \
    $FFMPEG_CFLAGS \
    "$SCRIPT_DIR/velox_media.c" \
    $FFMPEG_LIBS \
    -o "$OUTPUT_PATH"

echo "Built: $OUTPUT_PATH"

# -- 3. Cache to ~/.velox/cache/media/ -----------------------------------------
