...
# velox-media's verify.rs accepts it when VELOX_MEDIA_SKIP_VERIFY=1.
#
# Usage:
...
$Stem     = "velox-media-$Version-$Platform-$Arch"
$DllName  = "$Stem.dll"
$CacheDir = Join-Path $env:USERPROFILE ".velox\cache\media"
...
$Manifest = @{
    version = $Version
    url     = "https://cdn.velox.dev/media/$Version/$DllName"
    sha256  = $Hash
...
# 64 zero bytes = dummy signature (only valid when VELOX_MEDIA_SKIP_VERIFY=1)
[byte[]]$ZeroSig = ,0 * 64
...
Write-Host '  $env:VELOX_MEDIA_SKIP_VERIFY = "1"' -ForegroundColor Cyan
Write-Host "Then run: velox dev"
