...
# shared library so velox-media's verify.rs accepts it when
# VELOX_MEDIA_SKIP_VERIFY=1.
#
# Usage:
...
CACHE_DIR="$HOME/.velox/cache/media"
...
printf '{"version":"%s","url":"https://cdn.velox.dev/media/%s/%s.%s","sha256":"%s"}' \
...
echo "  export VELOX_MEDIA_SKIP_VERIFY=1"
echo "Then run: velox dev"
