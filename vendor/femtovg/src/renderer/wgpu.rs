...
/// Cache key for bind groups: the *resolved* textures (identity survives image-id
/// reuse and reallocation) plus the raw bits of the fragment uniforms.
#[derive(Debug, PartialEq, Eq, Hash)]
struct BindGroupCacheKey {
    main_texture: wgpu::Texture,
    glyph_texture: wgpu::Texture,
    uniform_bits: Vec<u32>,
}
...
pub struct WGPURenderer {
    ...
    bind_group_cache: Rc<RefCell<HashMap<BindGroupCacheKey, CachedBindGroup>>>,
}
...
impl Renderer for WGPURenderer {
    ...
    fn render(
        ...
        self.bind_group_cache
            .borrow_mut()
            .retain(|_, cached| std::mem::replace(&mut cached.accessed, false));
...
impl BindGroupState {
    /// Resolve this state to a cache key. Returns `None` when any referenced
    /// texture is external (those bind groups are not cacheable here).
    fn cache_key(&self, images: &ImageStore<Image>, empty_texture: &wgpu::Texture) -> Option<BindGroupCacheKey> {
        fn resolve(
            ...
            match image {
                Some(ImageOrTexture::Image(image_id)) => match images.get(*image_id).map(|img| &img.texture) {
                    Some(Texture::Internal(texture)) => Some(texture.clone()),
                    Some(Texture::External(_)) => None,
                    None => Some(empty_texture.clone()),
                },
                Some(ImageOrTexture::Texture(texture)) => Some(texture.clone()),
                None => Some(empty_texture.clone()),
            }
        }

        let main_texture = resolve(images, self.image.as_ref(), empty_texture)?;
        let glyph_texture = resolve(
            images,
            self.glyph_texture.image_id().map(ImageOrTexture::Image).as_ref(),
            empty_texture,
        )?;
        Some(BindGroupCacheKey {
            main_texture,
            glyph_texture,
            uniform_bits: bytemuck::cast_slice(self.uniforms.as_slice()).to_vec(),
        })
    }
...
impl CommandToPipelineAndBindGroupMapper {
    ...
    fn update_renderpass<'a>(
        ...
        if self.current_bind_group_state != Some(bind_group_state.clone()) {
            // Reuse a cached bind group (and its uniform buffer) whenever the same
            // draw state recurs — avoids re-creating hundreds of small uniform
            // buffers every frame. Entries not touched for a frame are evicted
            // after render(), matching the pipeline cache lifecycle.
            let bind_group = match bind_group_state.cache_key(images, &self.empty_texture) {
                Some(key) => {
                    let mut cache = self.bind_group_cache.borrow_mut();
                    let cached = cache.entry(key).or_insert_with(|| CachedBindGroup {
                        bind_group: bind_group_state.materialize(
                            &self.device,
                            images,
                ...
                None => bind_group_state.materialize(&self.device, images, &self.bind_group_layout, &self.empty_texture),
            };
