...
impl Renderer {
    ...
    /// Release all pooled GPU compute buffers (~100–170 MB on iGPU).
    ///
    /// Call this when the window is minimised or occluded.  Buffers are
    /// reallocated lazily on the next rendered frame (µs cost; only shader
    /// compilation is slow, and compiled shaders remain cached by the driver).
