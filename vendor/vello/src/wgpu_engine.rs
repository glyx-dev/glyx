...
impl WgpuEngine {
    ...
    /// Release all pooled GPU buffers.  See [`ResourcePool::trim`].
...
impl ResourcePool {
    ...
    /// Release all pooled GPU buffers.
    ///
    /// Call this when the window is minimised or occluded so the GPU driver
    /// can reclaim the memory.  Buffers are reallocated lazily on the next
    /// rendered frame (allocation is fast; only shader compilation is slow,
    /// and those remain cached).
